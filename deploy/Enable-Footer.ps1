...
.SYNOPSIS
  ...
  The footer is a fixed overlay shown on every page, revealed only when the page is
  scrolled to the bottom. Link columns come from the FooterLinks list (run
  Provision-IntranetLists.ps1 first); brand/copyright/accent/social are passed here.
...
.EXAMPLE
  .\deploy\Enable-Footer.ps1 -Url "https://contoso.sharepoint.com/sites/intranet" -ClientId "<app-id>" `
     -BrandText "Contoso" -Accent "#0f6cbd" `
...
if ($Remove) {
  try { Set-PnPFooter -Enabled:$true -ErrorAction Stop } catch {}   # restore the built-in footer
  Write-Host "Removed the Intranet Footer from $Url" -ForegroundColor Cyan
  return
}

# Our footer replaces the built-in site footer, so turn the built-in one off. (Safe: the
# footer is a body-mounted fixed overlay, it does not depend on the built-in footer.)
...
Write-Host "Intranet Footer enabled on $Url (accent $Accent). Reload any page and scroll to the bottom." -ForegroundColor Cyan
